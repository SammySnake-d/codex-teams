...
async fn multi_agent_v2_spawn_name_and_team_name_uses_teammate_branch() {
    ...
    match SpawnAgentHandlerV2::default().handle(invocation).await {
        ...
        Err(FunctionCallError::RespondToModel(message)) => {
            assert!(
                message.contains("requires a tmux or iTerm2 pane backend")
                    || message.contains("no `codex` binary with `codex teammate` support found")
                    || message.contains("CODEX_TEAMMATE_COMMAND does not support `codex teammate`"),
                "unexpected teammate branch failure: {message}"
            );
            assert!(
